    ...
    fn paint(&mut self, ctx: &mut PaintCtx, _data: &(), _env: &Env) {
        ...
        self.graph.visit_nodes(|node| {
            ctx.with_save(|ctx| {
                ...
                node.node_rect = Some(ctx.current_transform().transform_rect_bbox(rect).clone());
